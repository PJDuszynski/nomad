...
type CallbackServer struct {
	...
	clientNonce string
	errCh       chan error
	successCh   chan *structs.ACLOIDCCallbackRequest
}
...
func NewCallbackServer() (*CallbackServer, error) {
	...
	srv := &CallbackServer{
		...
		successCh:   make(chan *structs.ACLOIDCCallbackRequest, 5),
	}
...
func (s *CallbackServer) SuccessCh() <-chan *structs.ACLOIDCCallbackRequest { return s.successCh }
...
func (s *CallbackServer) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	// Build our result
	result := &structs.ACLOIDCCallbackRequest{
		State:       q.Get("state"),
